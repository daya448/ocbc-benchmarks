...
QUERIES_HYBRID_FILENAME: str = "queries_hybrid.json.bz2"
...
class HybridParamSource:
    def __init__(self, track, params, **kwargs):
        ...
        cwd = os.path.dirname(__file__)
        with bz2.open(os.path.join(cwd, QUERIES_HYBRID_FILENAME), "r") as queries_file:
            for vector_query in queries_file:
                self._queries.append(json.loads(vector_query))
        self._iters = 0
        self._maxIters = len(self._queries)
    ...
    def params(self):
        ...
        result["body"] = {
            "query": {
                "match": {
                    "text": self._queries[self._iters]["query"]
                }
            },
            "knn": {
                "field": "emb",
                "query_vector": self._queries[self._iters]["emb"],
                "k": self._params.get("k", 10),
                "num_candidates": self._params.get("num-candidates", 50),
            },
            "rank": {
...
def register(registry):
    ...
    registry.register_param_source("hybrid-param-source", HybridParamSource)
